...
for (const filename of ls(path.resolve(__dirname, 'waddle_geoportal/static-ngeo/js/apps/*.html.ejs'))) {
  const name = filename.file.substr(0, filename.file.length - '.html.ejs'.length);
  entry[name] = 'waddle/apps/Controller' + name + '.js';
  plugins.push(
...
const projectRule = {
  test: /waddle_geoportal\/static-ngeo\/js\/.*\.js$/,
  use: {
    loader: 'babel-loader',
    options: {
...
module.exports = {
  ...
  resolve: {
    ...
    alias: {
      waddle: path.resolve(__dirname, 'waddle_geoportal/static-ngeo/js'),
    },
  },
};
